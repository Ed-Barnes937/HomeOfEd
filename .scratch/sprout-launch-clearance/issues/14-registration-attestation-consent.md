...
- [ ] One committed drizzle-kit migration adds `uk_residence_attested_at timestamptz NOT
      NULL` and `tos_agreed_at timestamptz NOT NULL` to the `user` table (`NOT NULL` is
      safe — no deployed accounts exist).
- [ ] Both are Better Auth `additionalField`s following the `subscriptionStatus`
      precedent, carried in the signup payload.
- [ ] A `databaseHooks.user.create.before` hook rejects any signup lacking a true
      residence attestation **or** ToS agreement, and stamps both columns with
      **server** time (client-supplied timestamps ignored).
- [ ] The registration page gains two separate required checkboxes with the proposed,
...
- [ ] Unit tests over the sprout auth factory's signup API: unattested rejected,
      unagreed rejected, successful signup has both columns server-stamped.
- [ ] A whole-frontend test proves submit is blocked until both checkboxes are ticked.
- [ ] Verify loop green.
