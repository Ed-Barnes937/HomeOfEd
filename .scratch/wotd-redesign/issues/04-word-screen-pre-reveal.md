...
- [ ] Mobile pre-reveal matches design frame 5b (light and dark)
- [ ] Desktop pre-reveal matches design frame 5e (light and dark); the grid collapses to one column on tablet widths
- [ ] Level colour carries through the level pill, number badge, and primary button for all four levels
- [ ] "Hear it" plays the word, shows a playing state on the speaker icon, and is absent when speech is unsupported
- [ ] Back link returns to the picker; hit targets ≥44px
- [ ] `.iwft` coverage: word/type/respelling render for a seeded word; level colour carry-through; back navigation
