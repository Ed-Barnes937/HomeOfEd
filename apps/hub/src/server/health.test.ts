...
// sees a database.
class FakeHealthStore implements HealthStore {
  ping(): Promise<{ ok: true; value: string }> {
    return Promise.resolve({ ok: true, value: 'from the fake' })
  }
}

function makeCtx(store: HealthStore): AppContext<HealthStore> {
  return {
    store,
...
describe('HealthHandler', () => {
  it('returns ok plus the store-sourced value', async () => {
    const result = await new HealthHandler().run(undefined, makeCtx(new FakeHealthStore()))
