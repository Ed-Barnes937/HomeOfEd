/**
 * boids has no server-owned data (ADR 0006): the Store interface exists to
 * keep the layered skeleton and satisfy createAppServer's healthCheck seam,
...
export class InMemoryStatusStore implements StatusStore {
  ping(): Promise<{ ok: true; value: string }> {
    return Promise.resolve({ ok: true, value: 'flock is up' })
  }
}
