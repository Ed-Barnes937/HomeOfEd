...
// source — ADR 0004). The same router + handlers as dev/.iwft; boids has no
// database (ADR 0006), so the only "persistence" is the in-memory store.
...
const logger = createLogger().child({ app: 'boids' })
...
const server = createAppServer({
  ...
  logger,
  // "process up + serving" — there is no database to round-trip (ADR 0006).
  healthCheck: async () => {
    await store.ping()
    return { ok: true }
  },
