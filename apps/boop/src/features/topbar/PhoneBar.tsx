...
export interface PhoneBarProps {
  ...
  onClearGrid: () => void
  /**
   * Save the working grid into "My grooves" (ticket 20). Omitted until that
   * lands; the save icon then renders disabled rather than lying about what it
   * does.
   */
  onSave?: () => void
      ...
      {menuOpen && (
        <div className={styles.menu} data-testid="phone-menu">
          <button
            ...
            aria-disabled={onOpenMyGrooves === undefined}
            data-testid="phone-menu-my-grooves"
            onClick={onOpenMyGrooves === undefined ? undefined : withClose(onOpenMyGrooves)}
          >
            My grooves
          </button>
          ...
          <button
            type="button"
            className={styles.item}
            aria-disabled={onOpenHints === undefined}
            data-testid="phone-menu-hints"
            onClick={onOpenHints === undefined ? undefined : withClose(onOpenHints)}
          >
            How boop works
          </button>
          <button
