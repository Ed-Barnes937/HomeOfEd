...
test.describe('1280 — the first width the laptop numbers claim', () => {
  ...
  test('the lane grid fits too, at the clip cap', async ({ mountApp }) => {
    const { root } = await mountApp()
    await root.verifyIsShown()
...
test.describe('a vertical scrollbar must not start a sideways one', () => {
  ...
  // exactly, and a horizontal bar appeared under it. 700px tall with a full
  // song is where the lane grid scrolls vertically.
