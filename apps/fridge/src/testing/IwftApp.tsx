...
import { fridgeSchema } from '../server/schema.ts'
import { DrizzleFridgeStore } from '../server/store.ts'
...
exposeDispatcher(
  (async () => {
    ...
    const db = await applyPendingSeed(await freshTestDb(fridgeSchema, migrations))
    return createDispatcher({
      router: appRouter,
      createContext: createContext({
        store: new DrizzleFridgeStore(db),
        blobs: new InMemoryBlobStore(),
        logger: new ConsoleLogger({ app: 'fridge', mode: 'iwft' }),
        auth: testUserAuth,
      }),
    })
  })(),
