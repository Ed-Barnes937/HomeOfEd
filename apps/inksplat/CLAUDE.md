# apps/inksplat — scoped rules

A calm, single-screen, fully client-side canvas doodling toy at
`inksplat.homeofed.com`: procedurally-generated ink blots on warm paper that
...
`session.ts` below). Full spec: `.claude/tasks/inksplat-doodle/spec.md`;
ratified decisions: `.claude/tasks/inksplat-doodle/decisions.md`; divergences
from the design guide: [ADR 0016](../../docs/adr/0016-inksplat-doodle.md).
...
- `pnpm dev --filter=inksplat` — simulator mode on port **3006** (real router,
  no persistence; restart to pick up server changes).
- `pnpm test --filter=inksplat` — Vitest (`*.test.ts`) then Playwright CT
...
- **Persistence is the one exception, not a pattern:** `session.ts` mirrors the
  *current* drawing's `Op[]` to a single `localStorage` slot so an accidental
  ...
  server-owned data — see [ADR 0016](../../docs/adr/0016-inksplat-doodle.md).
- Undo is command-replay (an `Op[]` History), not raster snapshots — see
  `engine/history.ts` and [ADR 0016](../../docs/adr/0016-inksplat-doodle.md).
