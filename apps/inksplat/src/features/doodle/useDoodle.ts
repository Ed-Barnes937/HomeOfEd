...
export const DOODLE_SEAM_KEY = '__inksplatTestSeam'
...
export function useDoodle(): UseDoodle {
  ...
  useEffect(() => {
    ...
    const doSave = async (): Promise<void> => {
      ...
      if (!blob) return
      const file = new File([blob], 'inksplat.png', { type: 'image/png' })
      try {
        if (navigator.canShare?.({ files: [file] })) {
          await navigator.share({ files: [file], title: 'Inksplat' })
        } else {
          const url = URL.createObjectURL(blob)
          const anchor = document.createElement('a')
          anchor.href = url
          anchor.download = 'inksplat.png'
          anchor.click()
