...
export const DOODLE_SEAM_KEY = '__hiramekiTestSeam'
...
export function useDoodle(): UseDoodle {
  ...
  useEffect(() => {
    ...
    const doSave = async (): Promise<void> => {
      ...
      if (!blob) return
      const file = new File([blob], 'hirameki.png', { type: 'image/png' })
      try {
        if (navigator.canShare?.({ files: [file] })) {
          await navigator.share({ files: [file], title: 'Hirameki' })
        } else {
          const url = URL.createObjectURL(blob)
          const anchor = document.createElement('a')
          anchor.href = url
          anchor.download = 'hirameki.png'
          anchor.click()
