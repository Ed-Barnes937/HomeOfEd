# 0006 — inksplat mobile (Capacitor): initial-PR plan

- **Status:** Proposed
- **Date:** 2026-07-09
- **Related:** [ADR 0017](../adr/0017-inksplat-capacitor-mobile.md) (the
  decision this plan implements), [ADR 0016](../adr/0016-inksplat-doodle.md),
  [ADR 0008](../adr/0008-apps-without-a-database.md), the
  [phase-4 runbook](../runbooks/phase-4-go-live.md).

## 1. Summary

Wrap `apps/inksplat`'s existing web build in a Capacitor native shell so it can
...
Decisions already fixed by [ADR 0017](../adr/0017-inksplat-capacitor-mobile.md):

| Decision | Choice |
| --- | --- |
| Tool | **Capacitor**, wrapping the existing `vite build` — not a PWA-only path, not React Native. |
| Location | Inside `apps/inksplat` (`capacitor.config.ts`, later `ios/`, `android/`). |
...
- **Bundle ID** `com.homeofed.inksplat`, **app name** "Inksplat". Both are
...
All within `apps/inksplat` plus the two docs already written:
...
docs/adr/0017-inksplat-capacitor-mobile.md   (written)
docs/plans/0006-inksplat-capacitor-plan.md   (this file)
```
...
- **`capacitor.config.ts`**: `appId: 'com.homeofed.inksplat'`,
  `appName: 'Inksplat'`, `webDir: 'dist'`. `StatusBar`/`SplashScreen` plugin
...
pnpm test --filter=inksplat && pnpm build --filter=inksplat`.

- **C1 — ADR + plan.** [ADR 0017](../adr/0017-inksplat-capacitor-mobile.md) and
...
- **C4 — Config + safe areas.** `capacitor.config.ts`, `cap:*` scripts,
  ...
  *Verify:* loop green; **re-smoke the web build** (`docker compose up inksplat`
...
```bash
# On a Mac, from apps/inksplat, after the initial PR merges:
pnpm build --filter=inksplat          # produce dist/
...
- **Finalise the product name** before the listing is created — the bundle ID
  `com.homeofed.inksplat` is hard to change afterwards.

> **ℹ Still a placeholder.** `inksplat` / `com.homeofed.inksplat` are temporary
