# 0025 — boop: the versioned creation save format
...
- **Related:** [ADR 0008](0008-apps-without-a-database.md) (boop is stateless —
  ...
  Implements ticket 19; consumed by tickets 20 (My grooves) and 21 (share links).

## Context

Autosave is the first thing to write boop's state down, but two later features
read the same shape: "My grooves" (a named list a child saves into) and the
URL-hash share codec (the whole creation in a link, no server). The spec also
names pattern chaining — several patterns per creation — as the confirmed V2
...
2. **The creation is the unit.** A creation is
   ...
   creation, not the pattern, because a chained song plays at one tempo on one
   kit. Creations have no id: the list is index-addressed, as
   `apps/karesansui`'s presets already are.
3. **The working grid is a slot, not a list entry.** `working` holds the
   autosaved grid (an unnamed creation); `creations` is the saved list. Keeping
   them separate means autosaving cannot disturb a saved groove, and saving into
   "My grooves" is a copy of `working` with a name.
4. **A pattern is an object, rows are bitstrings.** `{ rows: [{ instrumentId,
   ...
   creation at a couple of hundred bytes, which matters for a share link that
...
5. **Decode is total, and strict.** `parseSaveDocument` never throws: missing,
   ...
   — one malformed creation discards the document rather than leaving a
   half-decoded list to be reasoned about later.
6. **Autosave is a lull, a ceiling, and a flush.** Writes wait 2 s of quiet
   (Groove Pizza's reference) so drag-painting a row is one write; a burst that
...
- Ticket 20 adds "My grooves" by writing the `creations` array — no format
  change, no migration. Ticket 21 encodes a `StoredCreation` (not a document)
  into the URL fragment and reuses `decodeCreation`'s defensiveness.
