# 0017 — inksplat mobile: Capacitor over the existing web build
...
- **Related:** [ADR 0003](0003-spa-default-tanstack-start-opt-in.md) (SPA
  ...
  [ADR 0016](0016-inksplat-doodle.md) (the doodle app itself),
  [plan 0006](../plans/0006-inksplat-capacitor-plan.md) (the implementation
  plan this ADR gates).

## Context

`apps/inksplat` is a stateless, fully client-side SPA: procedural canvas,
...
stores without abandoning the web version at `inksplat.homeofed.com`.
...
The step-by-step lives in [plan 0006](../plans/0006-inksplat-capacitor-plan.md).
...
2. **Capacitor lives inside `apps/inksplat`.** `capacitor.config.ts` and the
...
5. **The `server/` skeleton and web deploy are untouched.** Capacitor bundles
   ...
   `fly.toml`, the Dockerfile, `compose.yml`, and the `deploy-inksplat` CI job
...
- The placeholder name (ADR 0016, still `inksplat`) now also seeds the
  **bundle identifier** `com.homeofed.inksplat`. A bundle ID is hard to change
