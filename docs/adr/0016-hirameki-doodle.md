# 0016 — Hirameki doodle: divergences from the design guide
...
- **Related:** [ADR 0008](0008-apps-without-a-database.md) (stateless
  ...
  default), the epic's
  [`spec.md`](../../.claude/tasks/hirameki-doodle/spec.md) and
  [`decisions.md`](../../.claude/tasks/hirameki-doodle/decisions.md).

## Context

`apps/hirameki` (`hirameki.homeofed.com`) is a calm, client-side canvas
...
ADR is not about those. It builds from a `reference/hirameki/` design-guide
...
4. **Session restore of the current drawing** — the one softening of
   ...
   slot (`hirameki:doodle:v1`), overwritten as you draw; `useDoodle.ts` restores
